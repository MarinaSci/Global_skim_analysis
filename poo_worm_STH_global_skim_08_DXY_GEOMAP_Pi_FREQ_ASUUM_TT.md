...
### Frequency of alleles (pools) and SNPs (individuals)
```{r warning = FALSE}

SNP_frequency_function <- function(data) {
  SNP_data <- read.table(data, header = T)
  SNP_data2 <- SNP_data %>%
    mutate(SNP_frequency = number_variants/total_sites)
  country_map <- c(
    "BEN" = "Benin","CMR" = "Cameroon","TZA" = "Tanzania","ETH" = "Ethiopia","UGA" = "Uganda","IND" = "India","MWI" = "Malawi","MMR" = "Myanmar", "HND" ="Honduras",
    "NGA" = "Nigeria","ARG" = "Argentina","BGD" = "Bangladesh","CHN" = "China","ECU" = "Ecuador","GLP" = "Guadeloupe","FJI" = "Fiji","LKA" = "Sri Lanka",
    "MOZ" = "Mozambique","ITA" = "Italy","PR1" = "Puerto Rico","COD" = "DRC","SEN" = "Senegal","THA" = "Thailand","USA" = "U.S.A","ZAF" = "South Africa","MYS" = "Malaysia", 
    "KEN" = "Kenya")
  SNP_data2_country <- SNP_data2 %>%
    mutate(country = country_map[substr(sample.id, 1, 3)])
  
SAMPLE_TYPE_ALL_PER_SAMPLE <- read.table("/Users/marinapapaiakovou/Documents/00.Cambridge_PhD/02.Science/02.Genome_skimming/07.Global_genome_skim_2023/02_DATA/02_TRIMMED_DATA/04_VARIANT_CALLING/01_MITOGENOME_VARS/13_POPGEN_DATA_FOR_FIGURES/02_ASUUM_US_TT_DATA/sample_type_ALL_per_sample_ONLYSUUM82_SAMPLES_TT.txt"
                                           , header=T)
  #join the sample_type by species and sample.id
  SNP_data2_country2 <- left_join(SNP_data2_country, SAMPLE_TYPE_ALL_PER_SAMPLE, by = c("sample.id", "chrom"))
  }
# WORKS !!!!!
#TT_SNP_data <- SNP_frequency_function("TT_COUNTS_FOR_SNP_FREQ.txt")

INDV_SNP_FREQ <- SNP_frequency_function("/Users/marinapapaiakovou/Documents/00.Cambridge_PhD/02.Science/02.Genome_skimming/07.Global_genome_skim_2023/02_DATA/02_TRIMMED_DATA/04_VARIANT_CALLING/01_MITOGENOME_VARS/13_POPGEN_DATA_FOR_FIGURES/02_ASUUM_US_TT_DATA/INDV_SNP_FREQ_ASUUM_TT.txt")
INDV_SNP_FREQ$chrom[INDV_SNP_FREQ$chrom == 'NC_017750_Trichuris_trichiura_mitochondrion_complete_genome'] <- 'Trichuris trichiura'
INDV_SNP_FREQ$chrom[INDV_SNP_FREQ$chrom == 'NC_001327_Ascaris_suum_mitochondrion_genome_USA'] <- 'Ascaris suum - USA'
...
``` {r FIG2_POPGEN_part_SNP_frequencies_ASUUM_TT , fig.path = './00_FIGURES/'}

png(filename = "00_FIGURES/FIG2_POPGEN_part_SNP_frequencies_ASUUM_TT.png", height =3, width = 6, units = "in", res = 300)


INDV_SNP_FREQ_plot <- ggplot(data = INDV_SNP_FREQ, aes(x = country, y = SNP_frequency, color=chrom)) +
   #ggtitle("INDV - SNP frequency by country") +
  theme_bw() +
  facet_grid(. ~ chrom, scales = "free_x", space = "free_x", drop=TRUE) +
  geom_jitter(size = 2, alpha = 0.9, width=0.25) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1),legend.position = "none")+
  scale_color_manual(values = c("Trichuris trichiura" = "navyblue", "Ascaris suum - USA" = "orangered3"),
                     name = "chrom")+ylim(min(0), max(0.081))


POOLS_SNP_FREQ_plot <- ggplot(data = POOLS_SNP_FREQ, aes(x = country, y = SNP_frequency, color=chrom)) +
 # ggtitle("POOLS - SNP frequency by country") +
  theme_bw() +
  facet_grid(. ~ chrom, scales = "free_x", space = "free_x", drop=TRUE) +
  geom_jitter(size = 2, alpha = 0.9, width=0.25) +
 # theme(axis.text.x = element_text(angle = 90, hjust = 1),legend.position = "none")+
  theme(axis.text.x = element_text(angle = 90, hjust = 1),axis.text.y = element_blank(), legend.position = "none")+
  scale_color_manual(values = c("Trichuris trichiura" = "navyblue", "Ascaris suum - USA" = "orangered3"),
                     name = "chrom")+ylim(min(0), max(0.081))


FREQUENCIES_INDV_POOLS <- INDV_SNP_FREQ_plot + POOLS_SNP_FREQ_plot

print(FREQUENCIES_INDV_POOLS)
dev.off()
```
![FIG2_POPGEN_part_SNP_frequencies_ASUUM_TT](00_FIGURES/FIG2_POPGEN_part_SNP_frequencies_ASUUM_TT.png)
