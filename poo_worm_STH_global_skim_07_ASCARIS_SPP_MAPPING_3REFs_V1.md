# Resolving genetic distance of *Ascaris*-positive samples in the cohort - part I
...
reads_n <- read.table("/Users/marinapapaiakovou/Documents/00.Cambridge_PhD/02.Science/02.Genome_skimming/07.Global_genome_skim_2023/02_DATA/02_TRIMMED_DATA/04_VARIANT_CALLING/01_MITOGENOME_VARS/OLD_TESTS_TO_BE_CLEANED/KENYAN_CLUSTERS_POP1POP2PO3_EXPLORING/read_counts_for_all_samples_in_5X_ALUM_PCA_trimmed.txt", header=T, sep="\t")
...
png(filename = "00_FIGURES/Quantile_quality_plots_bcftools_ASUUM_82SAMPLES_NOMINIMUMALLELEFREQUENCY_10_95_Quantile.png", height = 4, width = 4, units = "in", res = 300)
...
#I have replaced sample.id with sample_id here before that's how it was in the original data with total number of reads
transform_dataset_run_BPCA_only <- function(file) {
  ...
  df_5 <- df_4[, c(2, 3, 7)]
  df_6 <- df_5 %>% tidyr::pivot_wider(names_from = sample_id, values_from = allele_frequency)
  ...
  df_8_bpca_dim1.dim2_sample_id <- tibble::rownames_to_column(df_8_bpca_dim1.dim2, "sample_id")
  # Define the country map
  country_map <- c(
    "BEN" = "Benin","CMR" = "Cameroon","TZA" = "Tanzania","ETH" = "Ethiopia","UGA" = "Uganda","IND" = "India","MWI" = "Malawi","MMR" = "Myanmar",
    "NGA" = "Nigeria","ARG" = "Argentina","BGD" = "Bangladesh","CHN" = "China","ECU" = "Ecuador","GLP" = "Guadeloupe","FJI" = "Fiji","LKA" = "Sri Lanka",
    "MOZ" = "Mozambique","ITA" = "Italy","PR1" = "Puerto Rico","COD" = "DRC","SEN" = "Senegal","THA" = "Thailand","USA" = "U.S.A","ZAF" = "South Africa","MYS" = "Malaysia", 
    "KEN" = "Kenya")
  df_8_bpca_dim1.dim2_sample_id_country <- df_8_bpca_dim1.dim2_sample_id %>%
    mutate(country = country_map[substr(sample_id, 1, 3)])
}
# THEY ARE ACTUALLY 81 SAMPLES !!!!!!!!!
...
png(filename = "00_FIGURES/BPCA_5X_nodups_HIGHQUAL_MAX_MISS_0.7_ASUUM_US_81SAMPLES.png", height = 5, width = 8, units = "in", res = 300)

ASUUM_US_82SAMPLES_PC1_PC2_PLOT <- ggplot(ASUUM_US_82SAMPLES_PC1_PC2, aes(PC1, PC2, color = country)) +
  ...
  ggtitle("ASUUM BPCA - 81 samples - no MFA applied")+
  theme_bw()

print(ASUUM_US_82SAMPLES_PC1_PC2_PLOT)
dev.off()
  
```
![BPCA_5X_nodups_HIGHQUAL_MAX_MISS_0.7_ASUUM_US_81SAMPLES.png](./00_FIGURES/BPCA_5X_nodups_HIGHQUAL_MAX_MISS_0.7_ASUUM_US_81SAMPLES.png)
- The lack of group clustering for all other countries/populations suggests that the primary sources of variation captured by the principal components are not strongly associated with country differences

- Colour by normalised reads and size by missingness to show that clustering is not attributed to either of them 
...
#run only the function up to the PC1/PC2 bit to access it later
ASUUM_US_82SAMPLES_PC1_PC2 <- transform_dataset_run_BPCA_only("//Users/marinapapaiakovou/Documents/00.Cambridge_PhD/02.Science/02.Genome_skimming/07.Global_genome_skim_2023/02_DATA/02_TRIMMED_DATA/04_VARIANT_CALLING/01_MITOGENOME_VARS/09_ASCARIS_SPP_VARS/03_ASUUM_US_82SAMPLES/ASUUM_US_82SAMPLES_bcftools_mtDNA_HIGHQUAL_n500_NOMINIMUMALLELEFREQ_nodups.recode_MAX_MISS_0.7_w_filtered_Indiv.FORMAT_AD.AD.FORMAT")
#ASUUM_US_82SAMPLES_PC1_PC2$sample_id <- sub("_trimmed", "", ASUUM_US_82SAMPLES_PC1_PC2$sample_id) #replace here, because the rest of the datasets do not have _trimmed
#WATCH OUT WITH THE ABOVE, I THINK YOU SHOULD KEEP THE TRIMMED, AS ALL OF THEM HAVE TRIMMED after the sample id
# THEY ARE ACTUALLY 81 SAMPLES !!!!!!!!!

```

```bash 
#need to calculate how many total reads mapped 
#run stats 
bedtools multicov -bed ASUUM_US.bed -bams *_filtered_CIGAR_final.bam > ASUUM_82SAMPLES_reads_mapped.txt
#bed file through here: awk 'BEGIN {FS="\t"}; {print $1 FS "0" FS $2}' NC_001327_Ascaris_suum_mitochondrion_genome_USA.fasta.fai > ASUUM_US.bed
#import the file here: 
```

- Import stats now into R 

```{r warning = FALSE}

ASUUM_82_SAMPLES_reads_mapped <- read.table("/Users/marinapapaiakovou/Documents/00.Cambridge_PhD/02.Science/02.Genome_skimming/07.Global_genome_skim_2023/02_DATA/02_TRIMMED_DATA/04_VARIANT_CALLING/01_MITOGENOME_VARS/09_ASCARIS_SPP_VARS/03_ASUUM_US_82SAMPLES/ASUUM_82SAMPLES_reads_mapped.txt")
colnames(ASUUM_82_SAMPLES_reads_mapped) <- c("species", "start_pos", "mitogenome_size_bp", "ECU_ALUM001_trimmed", "ETH001_trimmed", "ETH002_trimmed", "KEN_ALUM001_trimmed", "KEN_ALUM002_trimmed", "KEN_ALUM003_trimmed", 
                                  "KEN_ALUM004_trimmed", "KEN_ALUM005_trimmed", "KEN_ALUM006_trimmed", "KEN_ALUM007_trimmed", "KEN_ALUM008_trimmed", "KEN_ALUM009_trimmed", "KEN_ALUM010_trimmed", "KEN_ALUM011_trimmed", 
                                  "KEN_ALUM013_trimmed", "KEN_ALUM014_trimmed", "KEN_ALUM015_trimmed", "KEN_ALUM016_trimmed", "KEN_ALUM017_trimmed", "KEN_ALUM018_trimmed", "KEN_ALUM019_trimmed", "KEN_ALUM020_trimmed", 
                                  "KEN_ALUM021_trimmed", "KEN_ALUM022_trimmed", "KEN_ALUM023_trimmed", "KEN_ALUM024_trimmed", "KEN_ALUM025_trimmed", "KEN_ALUM026_trimmed", "KEN_ALUM027_trimmed", "KEN_ALUM028_trimmed",
                                  "KEN_ALUM029_trimmed", "KEN_ALUM030_trimmed", "KEN_ALUM031_trimmed", "KEN_ALUM032_trimmed", "KEN_ALUM033_trimmed", "KEN_ALUM034_trimmed", "KEN_ALUM035_trimmed", "KEN_ALUM036_trimmed", 
                                  "KEN_ALUM037_trimmed", "KEN_ALUM038_trimmed", "KEN_ALUM039_trimmed", "KEN_ALUM040_trimmed", "KEN_ALUM041_trimmed", "KEN_ALUM042_trimmed", "KEN_ALUM043_trimmed", "KEN_ALUM045_trimmed", 
                                  "KEN_ALUM046_trimmed", "KEN_ALUM047_trimmed", "KEN_ALUM048_trimmed", "KEN_ALUM049_trimmed", "KEN_ALUM050_trimmed", "KEN_ALUM051_trimmed", "KEN_ALUM052_trimmed", "KEN_ALUM053_trimmed",
                                  "KEN_ALUM055_trimmed", "KEN_ALUM057_trimmed", "KEN_ALUM058_trimmed", "KEN_ALUM059_trimmed", "KEN_ALUM060_trimmed", "KEN_ALUM061_trimmed", "KEN_ALUM062_trimmed", "KEN_ALUM064_trimmed",
                                  "KEN_ALUM065_trimmed", "KEN_ALUM067_trimmed", "KEN_ALUM068_trimmed", "MMR002_trimmed", "MMR007_trimmed", "MMR013_trimmed", "MMR016_trimmed", "MMR_NDK113_trimmed", "MMR_NDK63_trimmed", 
                                  "MMR_NKD92_trimmed", "MMR_TKU102_trimmed", "MMR_TKU23_trimmed", "MMR_TKU25_trimmed", "MOZ028_trimmed", "MOZ030_trimmed", "ZAF002_trimmed", "ZAF003_trimmed", "ZAF004_trimmed", "ZAF005_trimmed", "ZAF006_trimmed")

ASUUM_82_SAMPLES_reads_mapped_l <- pivot_longer(ASUUM_82_SAMPLES_reads_mapped, names_to = "sample_id", values_to = "raw_read_counts", cols=4:85)


#import mega sheet of total number of reads
...
#hum_samples_id_reads_n$sample_id <- sub("_trimmed", "", hum_samples_id_reads_n$sample_id)

# merged the dataframes
ASUUM_82SAMPLES_merged_data <- left_join(ASUUM_82_SAMPLES_reads_mapped_l, hum_samples_id_reads_n, by="sample_id")

ASUUM_82SAMPLES_merged_data_normalised <- ASUUM_82SAMPLES_merged_data %>% mutate(normalised = (raw_read_counts) / (raw_reads_n / 1000000) / (mitogenome_size_bp / 1e6))
#need to remove the _trimmed below

#import missingness data
ASUUM_US_82SAMPLES_imiss <- read.table("/Users/marinapapaiakovou/Documents/00.Cambridge_PhD/02.Science/02.Genome_skimming/07.Global_genome_skim_2023/02_DATA/02_TRIMMED_DATA/04_VARIANT_CALLING/01_MITOGENOME_VARS/09_ASCARIS_SPP_VARS/03_ASUUM_US_82SAMPLES/ASUUM_US_82SAMPLES_bcftools_mtDNA_HIGHQUAL_n500_NOMINIMUMALLELEFREQ_nodups.recode_MAX_MISSING_0.7.imiss", header=T) #NEED TO GENERATE AGAIN WITH CHROM COLUMN
#add a column that has chrom 
ASUUM_US_82SAMPLES_imiss$chrom <- "NC_001327_Ascaris_suum_mitochondrion_genome_USA"

#will replace the INDV with sample.id so i can match the datasets
colnames(ASUUM_US_82SAMPLES_imiss) <- c("sample_id", "N_DATA", "N_GENOTYPES_FILTERED", "N_MISS", "F_MISS", "chrom")
# Remove "_trimmed" from the sample_id using sub()
#ASUUM_US_82SAMPLES_imiss$sample_id <- sub("_trimmed", "", ASUUM_US_82SAMPLES_imiss$sample_id)

#MATCH THE DATASETS WITH MISSINGNESS AND COVERAGE 
country_map <- c(
...
#merge the datasets
ASUUM_82SAMPLES_merged_data_normalised_missingness <-  ASUUM_82SAMPLES_merged_data_normalised %>%
  left_join(ASUUM_US_82SAMPLES_imiss, by = c("sample_id")) %>%
  left_join(ASUUM_US_82SAMPLES_PC1_PC2, by="sample_id") %>%
  mutate(country = country_map[substr(sample_id, 1, 3)])

# Define breaks for the color scale
color_breaks <- c(100, 1000, 10000, 1000000)

#NEED TO REMOVE THE MOZ028 because it did not survive the filters!!
#i dON'T THINK IT'S BEING PLOTTED, BUT STILL, TO BE CONSISTENT
ASUUM_82SAMPLES_merged_data_normalised_missingness <- subset(ASUUM_82SAMPLES_merged_data_normalised_missingness, ASUUM_82SAMPLES_merged_data_normalised_missingness$sample_id != "MOZ028_trimmed")


```
- Plot the same PCA plot but colour by normalised reads and size by missingness

```{r, BPCA_ASUUM_US_81SAMPLES_mapped_colored_by_coverage_shaped_missingness, fig.path='./00_FIGURES'}

png(filename = "00_FIGURES/BPCA_ASUUM_US_81SAMPLES_mapped_colored_by_coverage_shaped_missingness.png", height = 7, width = 8, units = "in", res = 300)

#plot
plot <- ggplot(ASUUM_82SAMPLES_merged_data_normalised_missingness, aes(x = PC1, y = PC2, group = chrom, color = normalised, size = F_MISS)) +
  geom_point( alpha = 0.8) +
  #facet_wrap(~chrom, scales = "free") +
  #scale_colour_gradient(low = "purple4", high = "yellow2") +
  ...
  labs(
    x = "PC1",
    y = "PC2",
    title = "ASUUM - US BPCA plot 81 SAMPLES",
    subtitle = "Colored by Normalised, Sized by F_MISS"
  ) +
  theme_bw()
...
![BPCA_ASUUM_US_81SAMPLES_mapped_colored_by_coverage_shaped_missingness](00_FIGURES/BPCA_ASUUM_US_81SAMPLES_mapped_colored_by_coverage_shaped_missingness.png)


### Calculate new pairwise Dxy values for the samples that mapped best to the *Ascaris suum* ref (n=81)

- calling variants with pixy for individuals 
...
#filter for max alleles
vcftools --gzvcf ASUUM_US_82SAMPLES_bcftools_for_pixy.vcf.gz --max-alleles 2 --recode --recode-INFO-all --out ASUUM_US_82SAMPLES_bcftools_for_pixy_NOMINIMUMALLELEFREQ
#After filtering, kept 14279 out of a possible 14283 Sites

# will isolate now mtDNA genes and individuals
vcftools --vcf ASUUM_US_82SAMPLES_bcftools_for_pixy_NOMINIMUMALLELEFREQ.recode.vcf --bed ASUUM_US_mtDNA_genes_only.bed --keep ASUUM_INDV.txt --recode --recode-INFO-all --out ASUUM_US_82SAMPLES_bcftools_for_pixy_NOMINIMUMALLELEFREQ_mtDNA_genes_indvs
#After filtering, kept 65 out of 82 Individuals
#After filtering, kept 11925 out of a possible 14279 Sites
bgzip
tabix
#have 2 files with populations, ASUUM_US_INDV.pops2, where sample.id = population, and were population = country ASUUM_US_INDV.pops

#Run pixy with all the stats for the samples in the A.SUUM
pixy --stats pi --vcf ASUUM_US_82SAMPLES_bcftools_for_pixy_NOMINIMUMALLELEFREQ_mtDNA_genes_indvs.recode.vcf.gz --populations ASUUM_US_INDV.pops --window_size 20000 --n_cores 4 --bypass_invariant_check 'yes' --fst_type 'hudson' --output_prefix ASUUM_US_INDV_pi_output
pixy --stats dxy --vcf ASUUM_US_82SAMPLES_bcftools_for_pixy_NOMINIMUMALLELEFREQ_mtDNA_genes_indvs.recode.vcf.gz --populations ASUUM_US_INDV.pops2 --window_size 20000 --n_cores 4 --bypass_invariant_check 'yes' --fst_type 'hudson' --output_prefix ASUUM_US_INDV_dxy_output

```
- already have the variant VCF that will be used for the pooled samples
