# Theta relative Pi and Dxy  at different coverage thresholds for *Ascaris spp* using mitochondrial variants
Author: Marina Papaiakovou, mpapaiakovou[at]gmail.com 

## Contents: 
- Bash code to get individual Pi files with Grenedalf (from BAM files)
- R code to plot theta relative pi boxplots
- Bash code to calcualate mean depth per BAM 
- R code to plot mean depth to relative theta Pi 
- Bash code to calculate Dxy with Grenedalf 
- R code to plot Dxy heatmaps


### Theta relative Pi for pools of eggs; *Ascaris spp*
- Using Grenedalf with BAM files
...
````
- Move files to local machine to collate and plot in R 
...
- Plot theta pi at different coverage thresholds 

```{r REVIEWER_COMMENTS_Pi_COVERAGE_THRESHOLDs, fig.path='./00_FIGURES/'}
...
png(filename = "00_FIGURES/REVIEWER_COMMENTS_Pi_COVERAGE_THRESHOLDs.png", height = 6, width = 12, units = "in", res = 300)

# Basic boxplot of theta_pi_rel per coverage level
REVIEWER_COMMENTS_Pi_COVERAGE_THRESHOLDs <- ggplot(final_df, aes(x = factor(coverage), y = theta_pi_rel)) +
...
print(REVIEWER_COMMENTS_Pi_COVERAGE_THRESHOLDs)
dev.off()
```
![REVIEWER_COMMENTS_Pi_COVERAGE_THRESHOLDs.png](00_FIGURES/REVIEWER_COMMENTS_Pi_COVERAGE_THRESHOLDs.png)

- Calculating mean sample depth to plot with theta relative pi per sample (colour by country)
- To see if there are any associations between depth and pi 
...
```
- Move to R to curate the datasets
...
```{r REVIEWER_COMMENTS_Theta_Pi_MEAN_COVERAGE_PER_SAMPLE, fig.path='./00_FIGURES/'}
png(filename = "00_FIGURES/REVIEWER_COMMENTS_Theta_Pi_MEAN_COVERAGE_PER_SAMPLE.png", height = 6, width = 8, units = "in", res = 300)

REVIEWER_COMMENTS_Theta_Pi_MEAN_COVERAGE_PER_SAMPLE <- ggplot(COMBINED_Pi_MEAN_DEPTH, aes(x = mean_depth, y = theta_pi_rel, color = country)) +
...
print(REVIEWER_COMMENTS_Theta_Pi_MEAN_COVERAGE_PER_SAMPLE)
dev.off()

```
![REVIEWER_COMMENTS_Theta_Pi_MEAN_COVERAGE_PER_SAMPLE.png](00_FIGURES/REVIEWER_COMMENTS_Theta_Pi_MEAN_COVERAGE_PER_SAMPLE.png)


- Testing pooled Dxy (Grenedalf) using BAM files at different coverage thresholds 
...
```
- Pull between pi tables (=Dxy) to reformat in R 
...
cov40_dxy_pi_data_ASUUM_82SAMPLES_pools$coverage <- "40" #if needed, ended up using only up to the 20

cov80_dxy_pi_data_ASUUM_82SAMPLES_pools <- dxy_data_transformation("cov80_ALUM_pi_table_hudson_pi-between.csv")
cov80_dxy_pi_data_ASUUM_82SAMPLES_pools$coverage <- "80" #if needed, ended up using only up to the 20
...
# I only had 1 sample from Mozambique so I had to manually add Mozambique-Mozambique pair 
...
#to plot only 1,5, 10, 20 as I did for pi diversity 
...
- Plot now the heatmaps for between Pi (Dxy)

```{r REVIEWER_COMMENTS_Between_Pi_DXY_COVERAGE_THRESHOLDS_PER_COUNTRY, fig.path='./00_FIGURES/'}

#setwd("/Users/marinapapaiakovou/Documents/00.Cambridge_PhD/02.Science/02.Genome_skimming/07.Global_genome_skim_2023/03_CODE_CLEAN/GLOBAL_SKIM_ANALYSIS")

png(filename = "00_FIGURES/REVIEWER_COMMENTS_Between_Pi_DXY_COVERAGE_THRESHOLDS_PER_COUNTRY.png", height = 6, width = 12, units = "in", res = 300)


REVIEWER_COMMENTS_Between_Pi_DXY_COVERAGE_THRESHOLDS_PER_COUNTRY <- ggplot(
...
print(REVIEWER_COMMENTS_Between_Pi_DXY_COVERAGE_THRESHOLDS_PER_COUNTRY)
dev.off()

```
![REVIEWER_COMMENTS_Between_Pi_DXY_COVERAGE_THRESHOLDS_PER_COUNTRY.png](00_FIGURES/REVIEWER_COMMENTS_Between_Pi_DXY_COVERAGE_THRESHOLDS_PER_COUNTRY.png)
